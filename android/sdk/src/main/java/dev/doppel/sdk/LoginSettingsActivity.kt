...
class LoginSettingsActivity : Activity() {
    ...
    private fun render() {
        ...
        val header = LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL; setPadding(dp(12), dp(10), dp(20), dp(10)) }
        header.addView(UiTheme.icon(this, android.R.drawable.ic_media_previous, "返回") { finish() }, LinearLayout.LayoutParams(dp(44), dp(44)))
        header.addView(UiTheme.text(this, "登录辅助", 21f, bold = true)); root.addView(header)
        root.addView(UiTheme.divider(this))
        content = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(24), dp(22), dp(24), dp(24)) }
        ...
        try {
            ...
            content.addView(UiTheme.command(this, "保存手机号", true) { attempt { login.saveCommonPhone(phone.text.toString().trim()); Toast.makeText(this, "已保存到本机", Toast.LENGTH_SHORT).show() } }, LinearLayout.LayoutParams(-1, dp(46)).apply { topMargin = dp(12) })
            heading("验证码")
            ...
            if (profiles.isEmpty()) content.addView(UiTheme.text(this, "暂无授权应用", color = UiTheme.muted))
    ...
    private fun heading(title: String) { content.addView(UiTheme.text(this, title, 14f, UiTheme.muted, true).apply { setPadding(0, dp(22), 0, dp(12)) }) }
    ...
    private fun editProfile(existing: LoginProfile?) {
        ...
        val fields = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(20), dp(8), dp(20), dp(8)) }
        ...
        val enabled = Switch(this).apply { text = "允许自动填入手机号与登录验证码"; isChecked = existing?.enabled ?: false; minHeight = dp(56) }
