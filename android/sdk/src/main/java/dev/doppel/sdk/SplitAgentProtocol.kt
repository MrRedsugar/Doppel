...
internal object SplitAgentProtocol {
    ...
    val loginActions = setOf("login_password", "login_phone", "login_code")
    ...
    fun grounding(value: JSONObject, expectedAction: String): JSONObject {
        ...
        when(kind) {
            ...
            "login_password" -> out.put("package_name",text(value,"package_name",255)).put("credential_label",text(value,"credential_label",80))
            "login_phone", "login_code" -> out.put("package_name",text(value,"package_name",255))
    ...
    fun request(role: String, messages: JSONArray, direct: Boolean = false, expectedAction: String? = null): JSONObject {
        ...
        val popupHint = if (role == "primary") "\n先判断最前景弹窗，处理后重新观察再继续底页任务，不使用被弹窗遮挡的旧目标。界面不可读取不代表仍在加载；若用户遇到系统权限弹窗，请其手动选择后继续。" else ""
    ...
    fun grounder(image: String, intent: JSONObject): JSONObject {
        val instruction = JSONObject().put("action",intent.getString("action")).put("target",intent.getString("target"))
        ...
        return request("grounding",JSONArray()
            .put(JSONObject().put("role","system").put("content",GROUNDING))
            .put(JSONObject().put("role","user").put("content",JSONArray()
                .put(JSONObject().put("type","text").put("text",instruction.toString())).put(image(image)))),expectedAction=intent.getString("action"))
    }
...
浏览滑动（gesture_semantics=reveal_content）按以下顺序判断：
...
对象拖放（object_drag）按A的target中同一源对象→同一目标落点定位，目标不清或落点不可确定则拒绝，不替换对象或落点。A的方向仅是几何估计；target_relative_direction表示目标落点相对源对象的方向，必须与required_finger_direction同向。依据新截图细化任一方向字段（包括unknown变为具体方位）时必须direction_corrected=true，没有变化才false；坐标从源对象到目标落点，不套用浏览反向或small距离。连续手势逐段区分，对象拖放后另行选择朝向的手指运动仍是physical_gesture。
坐标 x/y 各自归一化到 0..1000，x 向右、y 向下增加，图片比例不变；points 按实际起点→终点顺序。普通浏览必须 small，沿移动轴约屏幕该轴的 5%–15%，接近目标可更短；large最多60%且仅用于明确直达边界。每段按 swipe_extent 选路径，不输出超幅坐标。低速拖动后松手，建议600–1000ms以减少惯性；短距离仍可能产生大位移，不快速甩动。宿主不会替你反转或缩放坐标。
只按本次 JSON Schema 输出 result。成功只返回执行字段和紧凑 assessment，不输出 observed、reason、correction_reason 等解释。alignment=consistent 仅表示执行前意图已核对（可含已纠正方向），不表示已经执行成功。duration_ms、interval_ms 等字段按 schema 填写。double_tap 一个点表示同处双击，两个点表示依次点击两处；type 原样返回 A 的 text；系统导航没有坐标。
目标按钮已不存在返回not_found，候选无法区分或浏览方向无法确定返回ambiguous，目标功能/页面关系矛盾返回intent_mismatch，不支持动作返回unsupported；拒绝仅返回status和简短reason，不返回可执行坐标。合理的中间导航无需已经显示最终结果，但不能为满足预期改点另一个目标。"""
}
